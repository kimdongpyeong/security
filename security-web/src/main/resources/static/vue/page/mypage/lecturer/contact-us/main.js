var MypageLecturerContactUsPage;
MypageLecturerContactUsPage = Vue.component("mypage-lecturer-contact-us-page", async function(resolve) {
    resolve({
        "template": (await axios.get("/vue/page/mypage/lecturer/contact-us/main.html")).data,
        "props": {
            roomId: {
                type: Number,
                default: 0
        ...
        "data": function() {
            return {
                ...
                "search": '',
                "status":"N",
                "userType":[{"value": "s", "label": "학생"},{"value": "p", "label": "학부모"}],
                "select":{
                	"type":""
                },
            }
        ...
        "methods": {
            ...
            "loadContactUsList": async function(){
            	...
            	await contactUsList.forEach(e => {
                    ...
                    this.$set(e, "roomName", e.userNm);
                    if(e.userProfile){
                        this.$set(e, "avatar", "/api/app/images?subpath=profile&filename="+e.userProfile);
                    } else{
...
//            	await this.setSelect(this.rooms[0]);
            ...
            "fetchMessages": async function(e) {
                ...
                await query.on('value', messages => {
                    ...
                    this.room.lastMessage = this.messages[this.messages.length-1];
                });
                this.rooms.forEach(x => {
                	if(x.id === this.roomId){
                		x.lastMessage = this.room.lastMessage;
                	}
                });
                document.querySelector('vue-advanced-chat').messages = this.messages
            },
